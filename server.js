...
app.get("/data", (req, res) => {
  const allData = "select * from employe_table";
...
app.post("/create", (req, res) => {
  ...
  try {
    connection.query(
      "INSERT into employe_table (name,email) values(?,?)",
      [name, email],
      function (err, result) {
        if (err) {
          console.log(err);
...
app.get("/delete-data", (req, res) => {
  const deleteData = "delete from employe_table where id=?";
...
app.get("/update-data", (req, res) => {
  const updateData = "select * from  employe_table where id=?";
  ...
  connection.query(updateData, req.query.id, (err, eachRow) => {
    if (err) {
      ...
      result = JSON.parse(JSON.stringify(eachRow[0]));  //in case if it did'nt work 
...
app.post("/update", (req, res) => {
  ...
  const updateQuery = "update employe_table set name=?, email=? where id=?";
